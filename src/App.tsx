...
// The LazyImage component generates images on demand via pollinations.ai
const LazyImage = ({ keyword, gender, uniqueName, description, className, autoLoad = false }: { keyword: string, gender: string, uniqueName: string, description?: string, className?: string, autoLoad?: boolean }) => {
  ...
  const generateImage = async () => {
    ...
    let hash = 0;
    const strToHash = k + uniqueName;
    for (let i = 0; i < strToHash.length; i++) {
        hash = strToHash.charCodeAt(i) + ((hash << 5) - hash);
    }
    const seed = Math.abs(hash) % 10000;
    
    const prompt = `High-end editorial portrait of a ${descriptor} modeling a specific haircut called "${uniqueName}" (${k} style), ${description ? `details: ${description}, ` : ''}professional salon photography, studio lighting, highly detailed hair texture, trending on pinterest, photorealistic, 8k resolution`;
    
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=800&height=1000&nologo=true&seed=${seed}`;
    ...
    <div className={`flex flex-col items-center justify-center bg-transparent text-white/90 border-r border-white/10 ${className || ''}`}>
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-2 px-2 text-center">
          ...
          <span className="text-red-500 text-[10px] uppercase font-medium">Сбой загрузки</span>
          <button 
...
export default function App() {
  ...
  const checkLimits = async () => {
    ...
    if (generationsLeft === null || generationsLeft <= 0) {
      setError('У вас закончились генерации. Вы можете пополнить их в настройках (⚙️).');
  ...
  const generateARPreview = async (styleKeyword: string, styleName: string) => {
    ...
    try {
      const response = await fetch('/api/generate-ar', {
        ...
        body: JSON.stringify({ imageBase64, mimeType, styleKeyword, styleName })
      });
      ...
      if (data.imageUrl) {
        // Pre-load the image to wait for pollinations.ai generation
        await new Promise((resolve, reject) => {
          const img = new Image();
          img.onload = () => {
  ...
  const loadMoreRecommendations = async () => {
    ...
    try {
      const response = await fetch('/api/load-more', {
        ...
        body: JSON.stringify({ imageBase64, mimeType: mimeType || "image/jpeg", existingNames })
      });
  ...
  return (
    <div className="min-h-screen bg-transparent text-white/90 font-sans text-white/90 selection:bg-blue-500/30">
      {/* Header */}
            ...
            <div className="mb-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-white/40 mt-2">
                  Если у вас есть пароль доступа (или собственный ключ от Google Gemini), вы можете ввести его здесь, чтобы обойти общие лимиты и воспользоваться приложением.
                </p>
              </div>
            </div>
